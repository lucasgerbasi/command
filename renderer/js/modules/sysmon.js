...
Modules.sysmon = {
  ...
  async render(container) {
    ...
    container.innerHTML = `${Utils.modHead('12 / System', 'System Monitor', '')}
    ...
    const update = async () => {
      try {
        ...
        const cpuPct = info.cpu;
        const ramPct = Math.round(info.ramUsed / info.ramTotal * 100);
        const diskPct = info.diskUsed !== undefined ? Math.round(info.diskUsed / info.diskTotal * 100) : null;
        ...
        grid.innerHTML = `
          <div class="sysmon-card">
            ...
            <div class="sysmon-sub">${info.cpuModel||''}</div>
          </div>
          <div class="sysmon-card">
            <div class="sysmon-card-label">RAM</div>
            <div class="sysmon-value">${(info.ramUsed/1024/1024/1024).toFixed(1)}<span class="sysmon-value-unit"> / ${(info.ramTotal/1024/1024/1024).toFixed(0)} GB</span></div>
          ...
          ${diskPct !== null ? `
          <div class="sysmon-card">
            <div class="sysmon-card-label">Disk</div>
            <div class="sysmon-value">${(info.diskUsed/1024/1024/1024).toFixed(0)}<span class="sysmon-value-unit"> / ${(info.diskTotal/1024/1024/1024).toFixed(0)} GB</span></div>
            <div class="sysmon-bar"><div class="sysmon-bar-fill ${barClass(diskPct)}" style="width:${diskPct}%"></div></div>
            <div class="sysmon-sub">${diskPct}% used</div>
          </div>` : ''}
          <div class="sysmon-card">
            <div class="sysmon-card-label">Uptime</div>
            <div class="sysmon-value" style="font-size:24px">${Utils.formatUptime(info.uptime)}</div>
        ...
        tbody.innerHTML = [
          ...
          ['Total RAM', (info.ramTotal/1024/1024/1024).toFixed(2) + ' GB'],
